...
/** @typedef {import('./apps').SakelarLain} SakelarLain */
...
export const MODUL_APPS_DATA = [
  {
    ...
    // Ringkasan lintas modul, READ-ONLY: kedua route-nya GET saja. BEKU tidak ditawarkan.
    hanyaBaca: true,
    punyaMenu: false,
  ...
  },
  {
    ...
    //
    // `menu`: cerminan sisi API — `realisasiMati()` menjaga seluruh `/api/blud/realisasi/*`
    // plus `bukti-setor` dan `pejabat`. `cetak` sengaja TIDAK ikut: layarnya tak memanggil
    // satu pun endpoint realisasi, dan mematikannya ikut mematikan cetakan DPA.
    subSakelar: [{
      kunci: 'app_status_blud_realisasi',
      label: 'Realisasi BLUD',
      menu: ['buku-kas', 'bukti-setor', 'realisasi', 'tutup-kas'],
    }],
    dirApi: 'app/api/blud',
 ...
 * sendiri di App Control. Tipe & penolongnya di `apps.ts`, isinya di sini.
 *
 * Ikut pindah ke berkas .mjs pada 2026-09-16 dengan alasan yang sama persis dengan
 * MODUL_APPS_DATA: gate G (`check:killswitch`) jalan dengan `node` biasa di CI, dan
 * selama daftarnya cuma ada dalam bentuk .ts, penjaganya tidak bisa membacanya sama
 * sekali. Itu yang membuat sakelar RIMA hidup bertahun-tahun tanpa satu pun route
 * memeriksanya sementara CI lulus terus — bukan karena gate-nya lemah, tapi karena
 * daftarnya tidak pernah sampai ke sana (T-1, pelajaran B5 lewat pintu kedua).
 *
 * `dijagaDi` menyebut HANDLER, bukan berkas. Gate G untuk modul sudah belajar itu di
 * Tahap 14d: `route.ts` yang mengekspor GET+POST lalu menjaga di GET saja tetap lulus
 * selama pemeriksaannya per berkas. Tiap handler yang diekspor di berkas terdaftar
 * wajib muncul di `metode` ATAU di `kecuali` — dibiarkan terbuka boleh, tidak disebut
 * sama sekali tidak.
 */

/** @type {readonly SakelarLain[]} */
export const SAKELAR_LAIN_DATA = [
  {
    kunci: 'app_status_sentinel_bot',
    label: 'RIMA (seluruh bot)',
    // Pemilik seluruh direktori route RIMA. Gate G menyapu isinya dan menuntut tiap
    // route.ts di sana muncul di `dijagaDi` — tanpa itu, route yang tidak didaftar
    // tidak diperiksa siapa pun, dan daftar penjaga yang terpisah dari daftar yang
    // dijaga persis cacat yang seluruh registry ini ada untuk membuangnya (T-1).
    dirApi: 'app/api/rima',
    dijagaDi: [
      { berkas: 'app/api/rima/lampir/route.ts', metode: ['POST'], penanda: ['rimaMati'] },
      {
        berkas: 'app/api/rima/feedback/route.ts',
        metode: ['POST'],
        penanda: ['rimaMati'],
        // GET & PATCH milik panel admin RIMA FEEDBACK, bukan bot. Orang mematikan RIMA
        // justru untuk memperbaikinya, dan membaca pertanyaan yang gagal dijawab adalah
        // cara memperbaikinya — sakelar pemeliharaan yang ikut membutakan panel
        // perbaikan membuat pemeliharaannya mustahil.
        kecuali: [
          {
            metode: ['GET', 'PATCH'],
      ...
      // Keduanya dijaga `rimaTanyaMati`, yang lewat `kunciSakelarLain` ikut membawa
      // kunci bot ini sebagai induknya — satu panggilan, dua sakelar.
      { berkas: 'app/api/rima/query/route.ts', metode: ['GET'], penanda: ['rimaTanyaMati'] },
      { berkas: 'app/api/rima/summary/route.ts', metode: ['GET'], penanda: ['rimaTanyaMati'] },
    ],
  },
  {
    kunci: 'app_status_rima_query',
    label: 'RIMA Tanya Data',
    // Sub-sakelar dari yang di atas: mematikan seluruh bot ikut mematikan Tanya Data,
    // tidak sebaliknya. Berjenjang seperti BLUD → Realisasi.
    induk: 'app_status_sentinel_bot',
    dijagaDi: [
      { berkas: 'app/api/rima/query/route.ts', metode: ['GET'], penanda: ['rimaTanyaMati'] },
      { berkas: 'app/api/rima/summary/route.ts', metode: ['GET'], penanda: ['rimaTanyaMati'] },
    ],
