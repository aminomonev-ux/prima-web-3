// app/api/kinerja/ssk/import/route.ts
// POST multipart (file xlsx) → baris tabel RKO (SSK). PEMBACA BERKAS SAJA.
//
// Tidak menulis DB sama sekali (pola IK-1/IK-4, dan L78 di modul BLUD): pencocokan
// ke baris RKO & Rekening yang sedang dibuka dihitung di klien, lalu penulisannya
// tetap lewat PUT /api/kinerja/ssk yang sudah ada — beserta kunci optimistiknya
// (`expected_version`) dan penolakan versi terkunci. Dua pintu tulis untuk data
// yang sama = dua set aturan, dan itu yang melahirkan lubang.
...
import { parseRkoImport } from '@/lib/data/kinerja-import-rko';
...
export async function POST(req: NextRequest) {
  ...
  const limited = await kinerjaRateLimit(session.userId, 'rko-import', 20);
  ...
  try {
    const hasil = await parseRkoImport(buf);

    await writeAuditLog({
      req,
      eventType: 'KINERJA_IMPORT_RKO',
      userId:    session.userId,
      username:  session.username,
      detail:    `Baca berkas RKO "${file.name}" (${hasil.rows.length} baris, ${hasil.source})`,
    });
