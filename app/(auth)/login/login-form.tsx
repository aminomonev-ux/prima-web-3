...
import type { Pemberitahuan } from '@/lib/security/pemeliharaan';

export default function LoginForm({ pemeliharaan }: { pemeliharaan: Pemberitahuan[] }) {
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .lp-pemeliharaan { display: flex; gap: 10px; align-items: flex-start; font-size: 12.5px; line-height: 1.6; color: #FFE08A; background: rgba(186,117,23,.20); border: 1px solid #FFC857; border-radius: 8px; padding: 10px 14px; margin-bottom: 14px; }
        .lp-pemeliharaan-ikon { font-size: 15px; line-height: 1.4; }
        .lp-pemeliharaan-baris + .lp-pemeliharaan-baris { margin-top: 6px; }
        .lp-pemeliharaan-pesan { white-space: pre-line; color: #E6F1FB; margin-top: 2px; }
        .lp-pemeliharaan-sampai { font-weight: 700; color: #FFC857; margin-top: 2px; }
        .lp-pemeliharaan-catatan { margin-top: 6px; color: #B5D4F4; }
        ...
        <div className="lp-right">
          <div className="lp-card fade-up">
            ...
            <div className="lp-card-body">
              ...
              {/* Fase F Tahap 17 (K2) — diumumkan sebelum masuk, dan login TETAP bisa.
                  Hanya pemeliharaan; modul beku masih bisa dibuka dan dicetak. */}
              {pemeliharaan.length > 0 && (
                <div className="lp-pemeliharaan" role="status">
                  <span className="lp-pemeliharaan-ikon">🔧</span>
                  <div>
                    {pemeliharaan.map((p) => (
                      <div key={p.label} className="lp-pemeliharaan-baris">
                        <b>{p.global ? 'Seluruh aplikasi' : p.label}</b> sedang dalam pemeliharaan.
                        {p.pesan && <div className="lp-pemeliharaan-pesan">{p.pesan}</div>}
                        {p.sampai && <div className="lp-pemeliharaan-sampai">Diperkirakan selesai {p.sampai}</div>}
                      </div>
                    ))}
                    {!pemeliharaan.some((p) => p.global) && (
                      <div className="lp-pemeliharaan-catatan">Anda tetap bisa masuk; bagian di atas belum bisa dibuka sampai pemeliharaan selesai.</div>
                    )}
                  </div>
                </div>
              )}
