...
function QuotaBar({used,limit,color}:{used:number;limit:number;color:string}) {
  ...
  const dangerColor = pct > 80 ? 'var(--ap-bad-fg)' : pct > 60 ? 'var(--ap-warn-fg)' : color;
  return (
    <div>
      <div style={{display:'flex',justifyContent:'space-between',fontSize:10,color:'var(--ap-dim)',marginBottom:4}}>
      ...
      <div style={{height:6,borderRadius:3,background:'var(--ap-aksen-bg)',overflow:'hidden'}}>
  ...
  async function save() {
    ...
    try {
      ...
      for (const [key,value] of entries) {
        ...
        if (!j.ok) { toast.error(j.message); return; }
      }
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,alignItems:'start'}}>
        ...
        <div className="ap-card">
          ...
          <div style={{marginBottom:20}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'10px 0',borderBottom:'1px solid var(--ap-line-tipis)'}}>
              <span style={{fontSize:13,color:'var(--ap-fg)',fontWeight:700}}>Email Notifikasi Aktif</span>
            ...
            <div style={{opacity:!isSA||!settings.enabled?.4:1,transition:'opacity .2s',pointerEvents:!isSA||!settings.enabled?'none':'auto'}}>
              <div style={{fontSize:10,color:'var(--ap-dim)',letterSpacing:.5,marginTop:14,marginBottom:4}}>USULAN</div>
          ...
          <div style={{marginBottom:16}}>
            <div style={{fontSize:11,color:'var(--ap-dim)',marginBottom:6,letterSpacing:.5}}>EMAIL PENERIMA ADMIN</div>
          ...
          {isSA && (
            <PrimaButton variant="primary" size="sm" iconLeft={<FileText size={12}/>}
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
            ...
            {quota ? (
              <>
                <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:16,padding:'10px 14px',
                  background:'var(--ap-aksen-bg)',border:'1px solid var(--ap-line)',borderRadius:8}}>
                  <Mail size={18} color="var(--ap-aksen)"/>
                  <div>
                    <div style={{fontSize:12,fontWeight:700,color:'var(--ap-fg)'}}>{quota.provider}</div>
                    <div style={{fontSize:10,color:'var(--ap-dim)'}}>Plan: <span style={{color:'var(--ap-ok-fg)',fontWeight:700}}>{quota.plan}</span></div>
                  </div>
                  <div style={{marginLeft:'auto',textAlign:'right'}}>
                    <div style={{fontSize:9,color:'var(--ap-dim)',letterSpacing:1}}>STATUS</div>
                ...
                <div style={{marginBottom:14}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                    <span style={{fontSize:11,color:'var(--ap-fg2)',fontWeight:600}}>Hari Ini</span>
                    <span style={{fontSize:10,color:'var(--ap-dim)'}}>Limit: {quota.dailyLimit}/hari</span>
                  </div>
                  <QuotaBar used={quota.sentToday} limit={quota.dailyLimit} color="var(--ap-aksen)"/>
                </div>

                <div style={{marginBottom:14}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:8}}>
                    <span style={{fontSize:11,color:'var(--ap-fg2)',fontWeight:600}}>Bulan Ini</span>
                    <span style={{fontSize:10,color:'var(--ap-dim)'}}>Limit: {quota.monthlyLimit}/bulan</span>
                  </div>
                  <QuotaBar used={quota.sentMonth} limit={quota.monthlyLimit} color="var(--ap-ok-fg)"/>
                </div>

                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
                  {[
                    ['Terkirim Hari Ini', quota.sentToday, 'var(--ap-aksen)'],
                    ['Sisa Hari Ini',     quota.dailyLimit-quota.sentToday, 'var(--ap-ok-fg)'],
                    ['Terkirim Bulan Ini',quota.sentMonth, 'var(--ap-aksen)'],
                    ['Sisa Bulan Ini',    quota.monthlyLimit-quota.sentMonth,'var(--ap-ok-fg)'],
                  ].map(([l,v,c])=>(
                    <div key={l as string} style={{textAlign:'center',padding:'8px 6px',
                      background:'var(--ap-aksen-bg)',border:'1px solid var(--ap-aksen-bg)',borderRadius:6}}>
                      <div style={{fontSize:20,fontWeight:400,color:c as string,fontFamily:"'JetBrains Mono',monospace"}}>{v as number}</div>
                      <div style={{fontSize:9,color:'var(--ap-dim)',letterSpacing:.5,marginTop:2}}>{l as string}</div>
                    </div>
                  ))}
                </div>
              </>
          ...
          <div className="ap-card">
            ...
            {[
              ...
              <div key={k as string} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid var(--ap-line-tipis)',fontSize:11}}>
            ...
            <table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}>
              <thead>
                <tr style={{borderBottom:'1px solid var(--ap-line)'}}>
                  <th style={{textAlign:'left',padding:'8px 6px',color:'var(--ap-dim)',fontWeight:600,letterSpacing:.5}}>WAKTU</th>
                  <th style={{textAlign:'left',padding:'8px 6px',color:'var(--ap-dim)',fontWeight:600,letterSpacing:.5}}>PENERIMA</th>
                  <th style={{textAlign:'left',padding:'8px 6px',color:'var(--ap-dim)',fontWeight:600,letterSpacing:.5}}>EVENT</th>
                  <th style={{textAlign:'left',padding:'8px 6px',color:'var(--ap-dim)',fontWeight:600,letterSpacing:.5}}>SUBJECT</th>
                  <th style={{textAlign:'center',padding:'8px 6px',color:'var(--ap-dim)',fontWeight:600,letterSpacing:.5}}>STATUS</th>
                </tr>
              </thead>
              <tbody>
                {logRows.map(r=>{
                  const badgeColor =
                    r.status==='SENT'             ? 'var(--ap-ok)' :
                    r.status==='FAILED'           ? 'var(--ap-bad-fg)' :
                    r.status==='SKIPPED_TOGGLE'   ? 'var(--ap-warn)' :
                    'var(--ap-dim)';
                  const ts = r.sent_at ? new Date(r.sent_at).toLocaleString('id-ID',{hour12:false}) : '-';
                  const hasErr = !!r.error_msg;
                  return (
                    <tr key={r.id} style={{borderBottom:'1px solid var(--ap-line-tipis)'}}>
                      <td style={{padding:'7px 6px',color:'var(--ap-fg2)',fontFamily:"'JetBrains Mono',monospace",fontSize:10,whiteSpace:'nowrap'}}>{ts}</td>
                      <td style={{padding:'7px 6px',color:'var(--ap-fg)',maxWidth:200,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{r.recipient}</td>
                      <td style={{padding:'7px 6px',color:'var(--ap-fg2)',fontFamily:"'JetBrains Mono',monospace",fontSize:10}}>{r.event_type}</td>
                      <td style={{padding:'7px 6px',color:'var(--ap-fg2)',maxWidth:280,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{r.subject}</td>
