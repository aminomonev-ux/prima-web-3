...
export default function UsulanClient({ userId, role, username, themePreference, beku }: Props) {
  ...
  async function doChangeRole(uid: number, role: string, alasan: string, putusSesi = false) {
    const d = await fetchJson('/api/admin/users', {
      ...
      body: JSON.stringify({ id: uid, action: 'ubah-role', role, alasan, putus_sesi: putusSesi }),
    });
