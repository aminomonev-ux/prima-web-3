...
CREATE TABLE IF NOT EXISTS pergeseran_dpa (
  ...
  bertambah           DECIMAL(18,2)     NULL COMMENT 'Uraian tangan: bagian yang MASUK. NULL = belum diuraikan, turunkan dari selisih (migration-pergeseran-uraian-geser)',
  berkurang           DECIMAL(18,2)     NULL COMMENT 'Uraian tangan: bagian yang KELUAR. NULL = belum diuraikan',
...
-- ─── BLUD — REALISASI (Buku Kas) ─────────────────────────────────────────────
-- migration-blud-realisasi-tx.sql · CONCEPT-blud-realisasi.md §6.2
-- Kolom turunan (pagu/terserap/sisa/persen/saldo berjalan/bulan lalu) TIDAK disimpan.
...
CREATE TABLE IF NOT EXISTS blud_realisasi_tx (
  ...
  jenis          ENUM('BELANJA','AMBIL_BANK','SETOR_BANK','PENERIMAAN','LAIN','PENGEMBALIAN') NOT NULL DEFAULT 'BELANJA',
  uraian         TEXT          NOT NULL,
...
CREATE TABLE IF NOT EXISTS blud_realisasi_alokasi (
  ...
  nilai          DECIMAL(18,2) NOT NULL COMMENT 'positif = membebani pagu; negatif = pengembalian belanja (jenis PENGEMBALIAN)',
...
-- Permintaan pergeseran / penambahan rekening (§4.1 & §4.2) — migration-blud-permintaan.sql
-- Tidak pernah menyentuh pagu; hanya mencatat permintaan supaya tidak hilang di WA.
...
CREATE TABLE IF NOT EXISTS rencana_aksi (
  ...
  target_rpjmd    DECIMAL(14,2) NOT NULL DEFAULT 0,
  target_tahunan  DECIMAL(14,2) NOT NULL DEFAULT 0,
  q1_target       DECIMAL(14,2) NOT NULL DEFAULT 0,
  q1_realisasi    DECIMAL(14,2) NOT NULL DEFAULT 0,
  q2_target       DECIMAL(14,2) NOT NULL DEFAULT 0,
  q2_realisasi    DECIMAL(14,2) NOT NULL DEFAULT 0,
  q3_target       DECIMAL(14,2) NOT NULL DEFAULT 0,
  q3_realisasi    DECIMAL(14,2) NOT NULL DEFAULT 0,
  q4_target       DECIMAL(14,2) NOT NULL DEFAULT 0,
  q4_realisasi    DECIMAL(14,2) NOT NULL DEFAULT 0,
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Modul IKI (Indikator Kinerja Individu) — docs/CONCEPT-iki.md
-- migration-iki.sql · varian STANDAR (11 kolom, 2 ttd) / DIREKTUR (8 kolom, 1 ttd)
-- ═══════════════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS iki_versi (
  ...
  COMMENT='IKI - riwayat versi (snapshot otomatis saat finalize/unfinalize) - migration-iki-versi.sql';
